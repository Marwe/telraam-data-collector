...
docs/data/
  ...
  device_XXXXXX/                  # Per-device directories
    2024-12.json                  # Monthly data files
    2025-01.json
    ...
    daily/                        # Daily aggregates (by month)
      2024-12.json                # Contains per-day totals for that month
      2025-01.json
```
...
`docs/index.html` is auto-generated after each collection run and lists links to all available `.json` files for easy browsing on GitHub Pages. View it live at https://maxliesegang.github.io/telraam-data-collector/.
...
2. **Storage** ([src/storage.ts](src/storage.ts))
   - Manages file operations
   - Groups data by month (YYYY-MM)
   - Merges new data with existing files
   - Prevents duplicates using date+hour as unique key
...
4. **Main** ([src/index.ts](src/index.ts))
   ...
   - Runs collection and reports results

## Data Deduplication

The system uses a combination of `date` and `hour` as a unique key to prevent duplicates. When new data is fetched:
